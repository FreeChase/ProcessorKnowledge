...
- [aarch64](#aarch64)
  ...
  - [cache](#cache)
    - [MESI协议](#mesi协议)
      - [一、MESI 协议的本质：**缓存层级中的一致性协议**](#一mesi-协议的本质缓存层级中的一致性协议)
        - [它的目标：](#它的目标)
      - [二、MESI 协议的限定范围（Scope）](#二mesi-协议的限定范围scope)
        - [换句话说：](#换句话说)
      - [三、MESI 协议作用的“域”——一致性域 (Coherency Domain)](#三mesi-协议作用的域一致性域-coherency-domain)
      - [四、为什么有限定范围：硬件成本与可扩展性](#四为什么有限定范围硬件成本与可扩展性)
      - [五、总结：MESI 的作用边界与不负责的部分](#五总结mesi-的作用边界与不负责的部分)
      - [六 存储 MESI 状态的结构](#六-存储-mesi-状态的结构)
        - [1. 缓存的内部结构](#1-缓存的内部结构)
        - [2. MESI 状态位的实现](#2-mesi-状态位的实现)
        - [3. 实际的缓存行结构（简化示例）](#3-实际的缓存行结构简化示例)
      - [一句话总结：](#一句话总结)
...
#### 三、MESI 协议作用的“域”——一致性域 (Coherency Domain)
